    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-100 flex flex-col">
      ...
      <section className="container mx-auto px-4 pb-8 max-w-3xl flex-grow-0">
        ...
        <div className="flex justify-center mt-6">
           <button onClick={() => setUseLLM(!useLLM)} className={`text-xs flex items-center gap-2 px-4 py-2 rounded-full transition-colors ${useLLM ? 'bg-indigo-50 text-indigo-600 font-medium' : 'text-slate-400'}`}>
      ...
      <section className="container mx-auto px-4 pb-24 max-w-4xl flex-grow">
        <AnimatePresence mode="wait">
          {isLoading ? (
            <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col justify-center items-center py-20">
            ...
            <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
              <div className="text-center bg-white p-8 rounded-3xl shadow-sm border border-indigo-50">
                <p className="text-2xl md:text-4xl font-serif text-slate-900 italic leading-tight">&quot;{result.copy}&quot;</p>
              </div>
              <div className="space-y-4">
                  {ladderSteps.map((step, index) => (
                    <motion.div key={step.step} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}>
              ...
              {result.social_post && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="bg-gradient-to-br from-indigo-600 to-violet-600 rounded-3xl p-1 shadow-xl">
                   <div className="bg-white rounded-[20px] overflow-hidden">
                        <div className="bg-indigo-50 p-4 border-b border-indigo-100 flex justify-between items-center">
                           <div className="flex items-center gap-2"><Sparkles className="text-indigo-600 w-5 h-5" /><span className="font-bold text-slate-900">Post Réseaux Sociaux</span></div>
                           <Button variant="ghost" size="sm" onClick={() => copyToClipboard(result.social_post!)}>{copied ? <Check size={16} /> : <Copy size={16} />}</Button>
                        </div>
                        <div className="p-8 whitespace-pre-wrap text-slate-700 font-medium">{result.social_post}</div>
                   </div>
                </motion.div>
              )}
            </motion.div>
      ...
      <footer className="py-8 text-center bg-white border-t border-slate-100 mt-auto">
        <a href="https://agencecible.fr" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-indigo-600 transition-colors font-medium text-sm">© {new Date().getFullYear()} Agence Cible</a>
      </footer>
    </div>
  );
}